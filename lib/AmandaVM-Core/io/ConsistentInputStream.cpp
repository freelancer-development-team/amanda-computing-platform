...
stream(const_cast<io::InputStream*> (stream.getSelfPointer()))
{
}
...
int ConsistentInputStream::read(void* buffer, size_t size) const
{
    return read(buffer, size, 1);
}

int ConsistentInputStream::read(void* buffer, size_t size, size_t count) const
{
    // Read the data and place it in the buffer
    int result = stream->read(buffer, sizeof (char), size * count);
    if (result == NO_ERROR)
    {
